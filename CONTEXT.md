...
**Hold**:
A human's instruction to leave a ready ticket alone: the `hold` label, which the dispatcher refuses whatever else the ticket carries. Removing it releases the ticket on the next sweep. Distinct from an **escalation** (the factory giving up) and from a blocker (the tracker's dependency edge): a hold is a person choosing the timing. `factory/lib/labels.ts` holds the set the dispatcher reads.
