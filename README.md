...
1. **Copy the caller.** `templates/factory.yml` goes to `.github/workflows/factory.yml` in the target. That file is the only factory file the target carries. Every job passes `factory_ref`, and it must equal the ref in that job's `uses:`, both `main` in the template. Change one without the other and they drift.
   ...
   Every target also needs one line for its own sessions, a line rather than a file: `templates/agents-md-judged-path.md` holds it, and it goes into the target's `AGENTS.md` (or `CLAUDE.md`) as it stands. A session that opens a PR on the target itself has to put `Closes #N` in the body, `agent:review` on the PR and auto-merge on it, or the PR sits blocked on `factory/verdict` for good. That line is how the session knows to do all three without being asked, and it is ADR 0003's judged path. Landing it is the target's own PR, and `scripts/onboard.sh` (step 3) prints the line again when it finishes.
