...
- Only the implementer's own failure spends the retry. A crashed reviewer (rate limited on every account, say), a push or PR step that failed, or a retry handler that could not reach GitHub keeps the `agent:blocked` comment for a human to re-label. A check still pending when the review's wait runs out (`CHECKS_TIMEOUT_MINUTES`, 15) counts as a failure so no PR sits unjudged.
