...
2. **Add the secrets.**
   ...
   **One token per target is one expiry date per target, and nothing in the factory watches them.** Record the date somewhere you will read it, because the notice is GitHub's own email to the token's owner before it lapses and there is no second one. Afterwards the symptom is every factory job on that one target failing on its first step, `Checkout factory`, since that step is where the token is first used; the dispatcher labels nothing, so tickets sit `ready-for-agent` and look held rather than broken. One target red while every other target keeps working is a lapsed token, not a factory bug. A target that will be down long enough to matter gets a waiver (below), which stops the factory's checks being required so the target keeps merging on its own CI.
