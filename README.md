...
3. **Run the onboarding script.** `scripts/onboard.sh owner/repo`, for example `scripts/onboard.sh chizhangucb/factory-fixture`. It creates the label vocabulary, allows auto-merge on the repo, and puts a `factory` ruleset on the default branch: PR required, squash only, and `factory/verdict`, `factory/red-green`, `factory/test-integrity` plus the target's own checks, all required on a head up to date with main. The own checks are the ones you name, `scripts/onboard.sh owner/repo check lint`; the script guesses none, ever. A first run that names none refuses and says the two ways on: name them, or pass `--no-own-checks` if the target genuinely has none yet, which onboards it with a warning that the ruleset gates on the factory's checks alone and a PR that breaks the target's build can still merge. Re-run the script to update the ruleset: a re-run naming nothing keeps exactly the own checks the ruleset already requires, so picking up a new label never needs that list remembered. Any run that would stop requiring one of them is refused, naming each check that would have been lost, unless you pass `--allow-drop`. Every refusal happens before any write: no label, no repo edit, no ruleset.
