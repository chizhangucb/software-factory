...
- A ticket labeled `agent:implement` by hand skips the dispatcher, so it skips the merge gate. That is deliberate: adding that label needs write access, and a maintainer doing it by hand is making the trust decision themselves. The reconciler re-adds the label on stranded work for the same reason, and the ticket it re-labels was dispatched under the merge gate in the first place.
...
- The linked issue's own title, body and, since #119, labels are read whatever their author's association, on all three paths. The labels join this list because implement-pr resolves the implementer model from them, so on a hand-labeled PR the linked issue's `model:` label picks the model the run spends, which is a model name rather than words an agent acts on. The issue number comes from the PR body's closing keyword, so this is not the dispatcher's vetted ticket by construction: on the factory's own PRs it is, because the dispatcher labeled that ticket, but a PR labeled by hand can link any issue. That is the hand-labeled case this ADR already records as deliberate, since the label needs write access and the person adding it is making the trust decision. Closing it properly needs the issue's own `author_association`, which `gh issue view --json` does not expose, so it is a second read and a behaviour change (a stranger's ticket would have no criteria and fail mechanically). Not done here; it is a follow-up, not a rationale. **Amended 2026-09-10 (#179).** Done, for the title and the body, exactly as this bullet describes: `fetchPullRequestContext` reads the ticket's `author_association` from REST beside the `--json` read, the `ticket-author` channel judges it, and an untrusted ticket reaches the reviewer, implement-pr and the audit with no criteria and a note in place of its body, which is the mechanical fail the bullet predicted. The verdict says which of the three no-criteria cases a PR is in, so a refused ticket is not reported as a ticket with no checklist. ADR 0003's 2026-09-10 amendment is why it moved from follow-up to now: the judged path is a second route that skips the dispatcher's author check. The labels are deliberately still read whatever the author's association, because writing one takes triage on the repo and a `model:` label is a model name rather than words an agent acts on.
...
- Shared, unchanged: the dispatcher decides what runs, the merge gate decides what may merge, the reviewer decides the verdict. None of the three runs a model over a sandbox; they read and write GitHub. A local path changes where `sandcastle.run()` happens, not who says go and who says ship.
