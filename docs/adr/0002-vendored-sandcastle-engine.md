...
Chronicle is the first public target (#20), and the consequence above says isolation is worth revisiting once the agent reads untrusted issue text under auto-merge. A public repo is exactly that: anyone can open an issue, and a ticket is the implementer's instructions. v0 still ships without a sandbox provider, for two reasons and with one control.
...
- Each heading now names its ticket, taken from the prose underneath it, so the ticket is not buried in a first sentence.
...
Prose was tightened where it repeated itself: the drop list, the dropped-count line and the run log became one bullet in the 2026-09-08 trusted-authors amendment, and the 2026-09-07 amendment's opening lost a re-quotation of the consequence it was already citing. The file is barely shorter for it, and that is the honest outcome. What made it long is the reasoning, and this ADR is a record: the reasoning is the part that may not go. What was actually wrong with it was the shape, five sections appended under two different words with no order to them, and the shape is what changed.
