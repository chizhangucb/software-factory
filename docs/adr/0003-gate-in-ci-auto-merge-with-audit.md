...
- The update happens in the target's workflow, so a target that does not carry the `push` and `status` triggers in its caller file stalls PRs behind main. `examples/factory.yml` has both. The `status` trigger creates a skipped run for every PAT-posted status; skipped jobs bill nothing.
