...
- A conflict the API cannot resolve gets a comment and `agent:blocked`; escalation proper is #16. Amended 2026-09-07 (#19): the first proof run stalled here, two concurrent heads had each appended to the target's README. A conflict now hands the PR to the implementer (`agent:implement` on the PR); implement-pr.yml detects it with `git merge-tree` and the agent merges `main` and resolves on the branch. The "LLM merger agent" rejection above stands for the routine case; true conflicts are exactly where it said an agent is needed.
