...
A requeue now means one thing on both sides: the comment naming the cause, no retry spent, no label. A ticket is left for the dispatcher, and a PR for the reconciler, which re-adds the start label at its `stuck_minutes` deadline, so the PR waits rather than queueing for a person. That covers both requeue causes, a rate limit and a head whose checks were still pending at the deadline, because they are one action in `factory/retry/retry.ts`. `agent-review.yml`'s own failure step tells the two apart by the file the run script writes: a rate limit on every account gets the same requeue comment and no label, and a crashed reviewer or any other failure still gets `agent:blocked` and its comment.
