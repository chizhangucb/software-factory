...
- Trade-off accepted: a GitHub concurrency group holds one running and one pending run. A third run arriving for a slot cancels the older pending one before any label moves, so the ticket stays on `agent:implement` with no run. The reconciler re-labels it at its stuck deadline on the next heartbeat sweep (#15 first gave this to the caller's cron, which was removed in #270; the heartbeat drives the sweep now). Slots are shared by implement, review, and implement-pr, so the cap counts every run kind.
...
`FACTORY_FORCE_RATE_LIMIT_ON` was scaffolding for the #19 proof run, which passed. It is gone from the scripts and from all four agent workflows, so nothing can make an attempt read as rate limited without running the agent. Rotation now has one path: `isRateLimited` on the run's own result events. The 2026-09-07 amendment on how the cap and the rate-limit detection shipped records the switch as it was built; this is what replaced it, not a correction of it.
...
- `Considered options` is new, and both entries come from the 2026-09-06 paragraph that opened this ADR: the rejection of importing claude-swap on the runner, and the ruling out of cc-switch. Each rejection's reason is word for word what that paragraph said. What changed is the framing around it, from running prose into a named bullet: "We considered importing the package on the runner and calling its pure functions, and rejected that:" is now "**Import claude-swap on the runner and call its pure functions.** Rejected:", and "cc-switch was ruled out:" is now "**cc-switch.** Ruled out:". Nothing new was weighed and no reason was added.
...
- Two cross-references that said "the note above" and "the amendment above" now name the amendment they mean by date and subject, since every appended section is an amendment now and two of them are dated 2026-09-07.
...
- The last bullet of the 2026-09-08 amendment was one 300-word paragraph covering the turn cap and both timeouts. Its sentences are unchanged word for word; they are now four sub-bullets, one per case, and the only edit is the colon that introduces them, where "The two land differently." was a full stop.
...
### 2026-09-10: the per-account cap is removed, and a subject key stays (#149)

The cap is gone. `per_account_slots`, the `slot` job and the `account-slot-<i>` groups are out of implement, review, implement-pr and audit, and out of the caller template. Rotation on a real rate limit is once again the only thing that moves a run off an account.

- What it cost. In one chronicle spec run at 20 lanes, four runs collided with an unrelated run holding the same lane. Two cost seconds, because the run holding the lane was already finishing. Two cost the remaining length of an unrelated implement run, about 10 minutes and about 40, while most lanes sat idle. A lane is the issue or PR number mod N over both kinds of number at once, so those collisions are arithmetic rather than load. Raising N thins them and never ends them.
- What it bought. Nothing that has been observed. No account has been rate limited at any point, including with six runs in flight on one account. The cap insured against a wall nobody has hit, and the premium came due four times in a single spec run.
- What stays: the one property the lanes got right, that two runs on the same subject never overlap. Each agent job's concurrency group is now the subject itself, `factory-issue-<n>` for implement and `factory-pr-<n>` for review, implement-pr and audit, with `cancel-in-progress` false. The role is not part of the key, so a review and an implement-pr run on one PR still serialise and a fix run never edits a branch a review run is reading. Two different subjects never share a key, so nothing waits behind a run it has nothing to do with. The audit's `decide` job keeps `factory-audit-counter`, which serialises the counter and was never the cap.
- The one-pending-per-group trade-off in the 2026-09-07 amendment on how the cap shipped survives in a narrower form. A third event on one subject still cancels the older pending run before it moves a label, but that run is now always one on the same subject, superseded by the newer one, and the reconciler's stuck path re-adds the label it left. Such a cancel costs no miss in practice, because the run that superseded it is the live run the reconciler finds, and the reconciler only repairs a subject with no live run past its deadline. If none survives, the subject really is stranded and the cancel counts as the lost event it is.
- The reconciler no longer reads a cancel cause out of a run's job names. With no lane to be contended, every cancel is an ordinary lost event and counts as a miss; its two-in-a-row hand-off to a human is unchanged.
- What is not built: a cap that picks a free lane by querying live runs. It was weighed and rejected. It is another number nobody can pick without data, it costs an API call per run, and it needs an answer for two runs picking the same free lane at once. No cap of any shape is built until a real rate limit is actually observed, and that first rate limit is the trigger to design one from data rather than from a guess.
- Removal is safe because a rate limit is no longer a human matter. #148 made a run rate limited on every account requeue on the PR side as it already did on the ticket side, so the worst case of no cap is wasted runs and waiting rather than a queue of PRs for a person.
- The 2026-09-07 amendment on headroom ranking calls the cap load-bearing for spreading load across accounts. It never spread load, as the 2026-09-08 amendment records: `pickToken` with no headroom returns the lowest account not rate limited, so runs pile onto account 1 whatever N is. Removing the cap changes how many run at once on the account in use, and nothing about which account a run lands on.
