...
- The turn cap went with it (#49). `implementer_max_turns` and `factory/lib/turn-cap.ts` are gone and the provider handed to `sandcastle.run()` is sandcastle's own, so the only stops left on a run are the implement script's 30 minute idle timeout and the 60 minute job timeout. The two land differently. The idle timeout is an `AgentIdleTimeoutError` raised inside `sandcastle.run()`, so `settleRun` sees a failed attempt, `runWithRotation` calls `fail()`, the step's outcome is `failure`, and the retry handler does run. The 60 minute job timeout cancels the job instead, and the retry handler is gated on `failure()`, so nothing downstream of it runs; #51 owns that path. `isRateLimited` is unaffected: a cap was never a rate limit.

## Amendment, 2026-09-08: the job timeout reaches the retry handler (#51)

The note above says the job timeout reaches nothing. It now does. The handler is no longer a step of the implement job, gated on `failure()`, which a cancelled job never satisfies; it is a job of its own that `needs` the implement job and runs `always()`, in `agent-implement.yml` and `agent-implement-pr.yml`. So the two bounds on a run land in the same place: the idle timeout as a `failure`, the job timeout as a `cancelled`, and `isImplementerFailure` in `factory/retry/decide.ts` counts both as the implementer's own failure, worth the ticket's one retry.

- The handler job runs on its own runner, so what the step read out of the implement job's context is passed explicitly: the branch, the implementer's outcome, the failure reason and the rate-limit marker as job outputs, the run log as the uploaded artifact.
- `agent-review.yml` keeps its handler as a step and needs none of this. It runs the handler on `success()`, after the verdict is posted, to judge the head's checks. A reviewer killed at the timeout posted no verdict, so there is nothing for it to judge; that PR is the reconciler's `verdict_minutes` case, which re-labels `agent:review` and escalates on a second miss.
