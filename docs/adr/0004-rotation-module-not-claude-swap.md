...
- The one-pending-per-group trade-off in the 2026-09-07 amendment on how the cap shipped survives in a narrower form. A third event on one subject still cancels the older pending run before it moves a label, but that run is now always one on the same subject, superseded by the newer one, and the reconciler's stuck path re-adds the label it left.
