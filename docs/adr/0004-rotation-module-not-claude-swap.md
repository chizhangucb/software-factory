...
- The turn cap went with it (#49). `implementer_max_turns` and `factory/lib/turn-cap.ts` are gone and the provider handed to `sandcastle.run()` is sandcastle's own, so the 60 minute job timeout is the only stop on a run. It stops the run without reaching the retry handler, which is gated on `failure()` and a timeout cancels the job; #51 owns that path. `isRateLimited` is unaffected: a cap was never a rate limit.
