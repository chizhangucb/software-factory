...
`FACTORY_FORCE_RATE_LIMIT_ON` was scaffolding for the #19 proof run, which passed. It is gone from the scripts and from all four agent workflows, so nothing can make an attempt read as rate limited without running the agent. Rotation now has one path: `isRateLimited` on the run's own result events. The 2026-09-07 amendment above records the switch as it was built; this is what replaced it, not a correction of it.
