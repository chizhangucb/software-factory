...
- The turn cap went with it (#49). `implementer_max_turns` and `factory/lib/turn-cap.ts` are gone and the provider handed to `sandcastle.run()` is sandcastle's own, so the only stops left on a run are the implement script's 30 minute idle timeout and the 60 minute job timeout. Neither reaches the retry handler: it is gated on `failure()` and a timeout cancels the job. #51 owns that path. `isRateLimited` is unaffected: a cap was never a rate limit.
