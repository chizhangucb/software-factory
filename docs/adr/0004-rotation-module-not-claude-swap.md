...
- The turn cap went with it (#49). `implementer_max_turns` and `factory/lib/turn-cap.ts` are gone and the provider handed to `sandcastle.run()` is sandcastle's own, so the 60 minute job timeout is the only stop on a run. `isRateLimited` is unaffected: a cap was never a rate limit.
