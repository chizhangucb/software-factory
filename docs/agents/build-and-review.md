...
3. **Review with the harness's own.** Invoke the bundled `code-review` with `medium --fix`. It hunts bugs and simplifications the first review is not looking for. A harness that bundles none has no step 3: an implementer run never sees it, since `factory/lib/harness.ts` renders the step off the provider the run drew, and a subagent says in its report that its harness had none. Done when its changes are read and the suite is green.
