...
- **Criteria go in the body**: a ticket moved to `ready-for-agent` carries its acceptance criteria in its own body, `/to-tickets` shape, under an ATX heading naming acceptance criteria with a checklist under it. `factory/lib/verdict.ts` is the one parser, and the dispatcher's gate, the reviewer and the audit all read it, so a brief posted as a comment is read by nothing and neither is a bold `**Acceptance criteria:**` line. This overrides the vendored `mattpocock-skills:triage` brief, which says both; the vendored files are not edited to match, being copied verbatim from the pin (`factory/plugins/README.md`).
