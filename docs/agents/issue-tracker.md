...
- **Criteria go in the body**: the checklist that gate reads is the reviewer's and the audit's too, one parser in `factory/lib/verdict.ts`: top-level checklist items under an ATX heading naming acceptance criteria, in the ticket's own body. A brief posted as a comment is read by nothing, and neither is a bold `**Acceptance criteria:**` line. The vendored `mattpocock-skills:triage` brief writes both; it is overridden here, never edited (`factory/plugins/README.md`).
