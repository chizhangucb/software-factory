...
- **Removing it releases the ticket on the next sweep, and a sweep runs within ten minutes.** Taking the label off wakes the dispatcher straight away, and the heartbeat sweeps every ten minutes even if that event goes missing. Nothing else has to be done to the ticket.
...
That is a hazard on the day a repo becomes a target, not on the day someone applies a label: every open `ready-for-agent` ticket arms at once, without anyone relabelling anything, so the backlog has to be swept (`hold`, or `ready-for-agent` off) before the caller lands. `docs/pipeline.md`'s onboarding details carry it, and it belongs to whoever adds the caller.
