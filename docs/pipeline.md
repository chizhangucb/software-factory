...
| `install_command` | merge-gate | `npm ci` | Empty to skip. Run once per checkout, so whatever a second kind of test needs installed (a browser, say) belongs here rather than in the test command. |
...
- `factory/red-green`: the PR's new or changed test files (`*.test.*`, `*.spec.*`, `_test.go`, `test_*.py`, anything under `__tests__/`) are copied onto a checkout of the base branch and run alone. At least one of them must fail there, and all of them must pass on the head. A helper or fixture under `test/` is a source change, not a test.
  ...
  So a target gets two layers, and only the first is owed to it. **The per-file split and passing over what could not be run protect every target for free**, with nothing to configure: no target is blocked by a check that lacks a command, and no passing file is ever blamed. **A routing test command is the opt-in second layer**, and what restores a real before-and-after proof for a second kind of test: `templates/routing-test-command.sh` is a worked example a target copies beside the caller, edits at its one marked mapping, and names in the merge-gate job's `test_command`. Only the target knows which of its files are which kind, so the factory ships the shape and not the knowledge. A target that never adopts it is correct, just less proved, and the status says so.
