...
| `test_command` | gate | `node --test` | A different input sharing the name: it receives the gate's test files as arguments rather than running the whole suite. It stays for the same reason as the row above, a target's test runner being a fact about the target, and without it the gate cannot run the new tests of a target whose runner is not `node --test`. |
| `install_command` | gate | `npm ci` | Empty to skip. |
| `per_account_slots` | implement, implement-pr, review, audit | `5` | Runs in flight per account, as `account-slot-<i>` concurrency groups. Setting it on dispatch, gate or update-branch fails the caller at parse time. It is an input because ADR 0004 asked for it, and ADR 0004 with its amendments is where the reasoning lives: what the cap is for, what the value should be, and the one-pending-per-group trade-off. |
...
- A run rate limited on every account is requeued, and a requeue means the same thing whichever side it is on (#148): a comment naming the cause, no retry spent, and no `agent:blocked`, because a quota is nobody's failure and no human can clear it. A ticket is left with no factory label, so the dispatcher re-dispatches it next sweep; a PR is left as it is for the reconciler, which re-adds its start label at the stuck deadline (`stuck_minutes`) and the run starts again. `agent-review.yml`'s own failure step reads the same fact the handler does, `rate_limited.txt` in the output dir (`RATE_LIMITED_FILE` in `factory/lib/accounts.ts`), and posts that same requeue comment with no label; every other failure there keeps `agent:blocked` and the comment above.
