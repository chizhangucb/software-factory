...
| `test_command` | review, audit | `npm ci && npm run typecheck --if-present && npm test` | Runs the target's whole suite before the agent starts; the output is evidence for the verdict. It is an input because how a target is built and tested is a fact about that target rather than a choice a maintainer could get wrong, the same class as `node_version`, and a target that is not an npm project would otherwise hand the reviewer and the audit no test evidence at all. |
| `test_command` | gate | `node --test` | A different input sharing the name: it receives the gate's test files as arguments rather than running the whole suite, because red-green runs the PR's new test files and nothing else, once on the base and once on the head. It is an input for the same reason as the row above, the target's test runner being a fact about the target, and without it the gate cannot run the new tests of a target whose runner is not `node --test`. |
| `install_command` | gate | `npm ci` | Empty to skip. |
| `per_account_slots` | implement, implement-pr, review, audit | `5` | Runs in flight per account, as `account-slot-<i>` concurrency groups. Setting it on dispatch, gate or update-branch fails the caller at parse time. It is an input because ADR 0004 asked for it, and that ADR is where the reasoning lives: its 2026-09-07 amendment on headroom ranking records the cap as load-bearing, its 2026-09-08 amendment moves the default from 3 to 5 and says why, and the first also records the one-pending-per-group trade-off. |
...
- A serialized `decide` job asks whether this merge is one of the first 20 (`AUDIT_LIMIT` in `factory/audit/plan.ts`, the one place the number is written; the job publishes it as an output so no workflow file carries it): the PR must be merged and a factory PR (`CONTEXT.md` defines that; `factory/lib/factory-pr.ts` is the one definition, which the reconciler reads too), and the counter must be under the limit. Past the limit the job logs `audit limit reached` and exits.
...
- `factory/gate/`, `factory/dispatch/`, `factory/update-branch/`, `factory/retry/`, `factory/audit/`: one folder per factory-owned job, `retry/` being the `retry` job of the two implement workflows rather than a workflow of its own. Each keeps its workflow's decisions as pure, `node --test` covered functions, and the script beside them only gathers inputs and calls GitHub: `dispatch/select.ts` and `dispatch/reconcile.ts`, `update-branch/plan.ts`, `retry/decide.ts` and `retry/escalation.ts`, `audit/plan.ts`, and the modules under `gate/` that `gate.ts` runs. The dispatch, update-branch and audit-decide jobs install nothing and run on Node's type stripping, so a 10-minute sweep costs seconds, billed as a minute on private repos.
