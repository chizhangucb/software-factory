...
| `templates/` | `factory.yml`, the caller a target copies, and `routing-test-command.sh`, the worked routing test command a target with two kinds of test copies into its own tree and names in its caller (#161). Both files are ours; `templates/` is sandcastle's word for the folder |
