...
| `templates/` | `factory.yml`, the caller a target copies; `routing-test-command.sh`, the worked routing test command a target with two kinds of test copies into its own tree and names in its caller (#161); and `agents-md-judged-path.md`, the one line every target puts in its `AGENTS.md` so a session opening a PR itself takes the judged path (#181). All three files are ours; `templates/` is sandcastle's word for the folder |
