...
| update-branch, API and no agent (`factory/update-branch/`) | 11; ADR 0003 amendment | No for the routine case: his update-branch is an agent on a label, ours an API call on push with a verdict carry. The conflict case is the "Conflict resolution reinvented in implement-pr" entry in `docs/provenance/sandcastle-files.md`. |
