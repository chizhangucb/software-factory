...
The library is a dependency, unmodified. `package.json` pins it to exactly `0.12.0`; `package-lock.json`'s `integrity` for `node_modules/@ai-hero/sandcastle` is the live check `npm ci` verifies every run; the tarball is cold storage on this repo's `engine-0.12.0` release and nothing fetches it (ADR 0002's 2026-09-08 amendment).
...
Only the prompt counts have moved since, never a script row, because #54 and #56 touched prose and comment text and this method excludes comments. #47 put his section skeletons back, taking `implement/prompt.md` from 3 surviving lines to 7 and `review/prompt.md` from 12 to 14. #54 moved no line of his, leaving `implement/prompt.md` shorter (42 lines to 40) and `implement-pr/prompt.md` one longer. #56's pass cost him five lines in `implement-pr/prompt.md`, 89 percent down to 71, its largest drop; the other two prompts kept every line of his, since what survives in them is headings, placeholders, code fences and his closing promise line. Both `extraction.md` files were left alone as a format contract, and `implement-pr/extraction.md` is still the only vendored file that is his to the line.
