...
This repo is not a fork of [mattpocock/sandcastle](https://github.com/mattpocock/sandcastle), and it is not an original pipeline either. Three of his GitHub Actions workflows, their scripts and their prompts were copied in and extended; his npm library is a pinned dependency, unmodified; everything else was written here. This document says which is which, file by file, and it is the evidence for ADR 0002 (`docs/adr/0002-vendored-sandcastle-engine.md`), which cites it.
...
The library is consumed as a dependency, unmodified. `package.json` pins `@ai-hero/sandcastle` to exactly `0.12.0`; `package-lock.json`'s `integrity` for `node_modules/@ai-hero/sandcastle` is the live check `npm ci` verifies on every run; the 0.12.0 tarball is cold storage on this repo's `engine-0.12.0` release and nothing fetches it (ADR 0002's 2026-09-08 amendment).

Nothing from `src/` was copied. Re-verified on 2026-09-08 by grepping all 161 exported names of his `src/*.ts` against `factory/`: the only matches are `run`, `claudeCode` and `Output`, all reached through the package, plus `create` and `remove` in unrelated contexts. Add `noSandbox`, a sub-path export (`@ai-hero/sandcastle/sandboxes/no-sandbox`) absent from that top-level list, and the whole contract with the library is four names and a few types, the contract `docs/research/sandcastle-inventory-2026-09.md` proposed:
...
Every line differing from his carries a comment naming the ticket or ADR that forced it (#47). A vendored `.md` carries none of its own, because a comment in a prompt is context the model reads; the sibling script's header accounts for the prompt's differences too.
...
- **`agent-update-branch.yml` and `.sandcastle/agent-workflows/update-branch/`** (`update-branch.ts`, `prompt.md`, `extraction.md`). Left out because #9's Out of Scope said "the update-branch agent for true conflicts... conflicts escalate in v0", and #21 agreed. #19 then reversed that and built conflict resolution into implement-pr rather than copying his, which makes this the largest avoidable rewrite in the vendoring. Section 8's first entry carries the decision that keeps it as built.
...
- **Removed everywhere**: **Install dependencies** and **Build**, which exist only so his scripts can self-reference his own `dist/` after a build. The factory installs the pinned package instead, and **Install Claude Code** folds in with them as one step, "Install factory (pinned sandcastle, tsx, Claude Code)", installing from this repo's lockfile rather than `npm i -g` unpinned (#21, `8f0c3f1`).
...
Five of the eight scripts are over 90 percent his. The three that are not are the three the spec changed most: the implementer gained a whole ticket document and a rotation wrapper, the reviewer stopped being a writer, and `review-context.ts` became the place trust is enforced for three agents at once (#52), moving most recently and by the largest step, 97 percent to 69.

Only the prompt counts have moved since, and never a script row, because #54 and #56 touched prose and comment text and this table's method excludes comments. #47 put his section skeletons back, taking `implement/prompt.md` from 3 surviving lines to 7 and `review/prompt.md` from 12 to 14. #54 turned the prose that imitated Matt's TDD and review skills into calls to those skills by name and moved no line of his, leaving `implement/prompt.md` shorter (42 lines to 40) and `implement-pr/prompt.md` one line longer; the content inside the sections is still ours and has to be, because stories 4, 5, 7, 8 and 23 all land in prompt text. #56's writing pass (story 14 of #46) cost him five lines in `implement-pr/prompt.md`, 89 percent down to 71, its largest drop: his prose there had survived almost whole and the pass rewrote it. The other two prompts kept every line of his, because what survives in them is his section headings, template placeholders, code fences and closing promise line, which a prose pass had no reason to touch. Both `extraction.md` files were left alone on purpose, being a format contract rather than prose, and `implement-pr/extraction.md` is still the only vendored file that is his to the line.
...
|---|---|---|
| dispatch (`factory/dispatch/`) | 1, 2, 21, 28, 30; #15 | No. His pipeline has no dispatcher; a human applies the label, and his `simple-loop` template lets an agent pick issues, which #9 excludes ("any planner that invents work"). Story 21 is the exception: the `agent:*` label vocabulary the dispatcher writes is entirely his (section 9), and only the thing applying it is new. |
