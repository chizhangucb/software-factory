...
4. A criterion is met only when the code does the work for the cases the criterion describes, not only for the cases the tests happen to cover. A placeholder does not count: a stub, a hardcoded return, a test that asserts the stub, a skipped or weakened test, a deleted test the ticket does not remove, a branch the criterion needs that is missing or wrong. A deleted test is judged against what the ticket says it removes, wherever the ticket says it; a deletion the ticket does not account for is a placeholder. Report every placeholder you find in `placeholders`, with the file and line.
