...
import { deletedTestFiles, type Verdict } from "./test-integrity";
 ...
 * file while adding or changing no test, both pass vacuously: the second
...
export const redGreenPlan = (files: readonly ChangedFile[]): RedGreenPlan => {
  ...
  if (files.some((f) => f.status === "D") || deletedTestFiles(files).length > 0) {
