import { deletedTestFiles, type ChangedFile, type FileKind } from "./changed-files";
 ...
 * ticket. A diff that changes no source file, and a diff that deletes any
 * file (a test renamed out of the test tree counts as its deletion) while
 * adding or changing no test, both pass vacuously: the second
 * says in words that nothing was proved, because a removal has no new test
 * to go red on the base, and whether the removal was owed is the reviewer's
 * and the audit's call. Accepted as a false negative: a source change that
 * also deletes any file slips this check and meets the reviewer; the same
 * change refused here would meet nobody. A source change with no deleted
 * file and no test change is the placeholder shape this check exists to
...
export const redGreenPlan = (files: readonly ChangedFile[]): RedGreenPlan => {
  ...
  const deletesAFile = files.some((f) => f.status === "D") || deletedTestFiles(files).length > 0;
  if (deletesAFile) {
