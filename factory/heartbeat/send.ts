 ...
 * so a host's own log keeps the history and its own alerting sees the non-zero
 * exit a failed target ends on. Where it runs is #111.
 *
...
const outcomes = sendHeartbeat({
  ...
  report: (outcome) => {
    ...
    if (outcome.outcome === "failed") console.error(`factory-sweep FAILED for ${outcome.target}: ${outcome.error}`);
    else console.log(`factory-sweep dispatched to ${outcome.target}${dryRun ? " (dry run)" : ""}`);
  },
});

const failed = outcomes.filter((outcome) => outcome.outcome === "failed");
console.log(`${outcomes.length} target(s), ${outcomes.length - failed.length} woken, ${failed.length} failed${dryRun ? " (dry run)" : ""}.`);
